...
namespace Weborama\Database\QueryBuilder;
...
abstract class Base implements \IteratorAggregate
{
     ...
     * @param $clauses
     */
    protected function __construct(Database $db, $clauses)
    {
        $this->db  = $db;
    ...
    public function execute()
    {
        ...
        $result = $this->db->getPdo()->prepare($query);
        ...
        } elseif ($this->db->getPdo()->getAttribute(\PDO::ATTR_DEFAULT_FETCH_MODE) == \PDO::FETCH_BOTH) {
            ...
            if (!is_callable($this->db->debug)) {
                ...
                foreach (debug_backtrace() as $backtrace)
                {
                    if (isset($backtrace['file']) && !preg_match($pattern, $backtrace['file'])) {
                        // stop on first file outside Database source codes
                        break;
                    }
                }
                ...
                call_user_func($this->db->debug, $this);
            }
        }
    }
